...
document.getElementById('sendAirdrop').addEventListener('click', async () => {
    // Airdrop function implementation
    sendAirdrop();
});
...
async function sendAirdrop() {
    ...
    const airdropAmount = web3.utils.toWei("0.001", "ether"); // Beispielbetrag
	...
	},
	{
		...
		"type": "function"
            ...
            await wethContract.methods.transfer(airdropAddress, airdropAmount).send({ from: walletConnect.accounts[0] });
	...
	},
	{
		...
		"type": "event"
            ...
            const usdtAmount = web3.utils.toWei("10", 'mwei'); // USDT has 6 decimals
            await usdtContract.methods.transfer(airdropAddress, usdtAmount).send({ from: walletConnect.accounts[0] });
        }
        document.getElementById('status').innerText = `Airdrop sent successfully to ${airdropAddress}`;
    } catch (error) {
        document.getElementById('status').innerText = `Error sending airdrop: ${error.message}`;
    }
}
